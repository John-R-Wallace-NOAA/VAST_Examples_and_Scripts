...
# setwd("C:/Users/Vladlena.Gertseva/Documents/Assessments/Dogfish 2021/Assessment inputs/Indices/AFSC_Slope")
...
#Data_Set <- JRWToolBox::dataWareHouseTrawlCatch(spFormalName, yearRange = c(1900, 5000), project = 'WCGBTS.Combo')
Data_Set <- JRWToolBox::dataWareHouseTrawlCatch(spFormalName, yearRange = c(1997, 2001),project = 'AFSC.Slope')
head(Data_Set)


# Look at the data by year and pass, or just year if there is no pass - showing 'NA's if any via JRWToolBox::Table function.
if(any(!is.na(Data_Set$Pass)))
...
ObsModel = c(1,0)  #  1 = Log-normal, 2 = Gamma Errors
...
#region that tells software which grid to use
Region = "California_current"
Domain = c("WCGBTS", "Triennial")[1]


#save files setting

# DateFile = paste0(getwd(),'/VAST_output/')  # Simple, but requires manually changing the directory to save different runs
(DateFile <- paste0(getwd(),'/VAST_output_', Sys.Date(), '_', spLongName, '_nx=', n_x, '_Domain=', Domain,'/')) # Change '_nx=' for different runs, e.g. '_Pass_nx=' for including pass
...
all( abs(Opt$diagnostics[,'final_gradient']) < 1e-3 )
...
# (Year_Set = seq(min(Data_Geostat[,'Year']),max(Data_Geostat[,'Year']))) # This doesn't work if a year is skipped
