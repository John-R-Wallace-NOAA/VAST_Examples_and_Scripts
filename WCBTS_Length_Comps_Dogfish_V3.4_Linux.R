...
# Set max threads if using MRO on a Linux server running CentOS (e.g. Tantalus), if using R ver 3.5.3 
# setMKLthreads(6); getMKLthreads() 

# Set max threads if using R-MKL on a Linux server running CentOS (e.g. Tantalus), if using R ver 4.X
...
if(numSexInModel %in% 2)
    LenCompNoZero <- LenCompNoZero[!LenCompNoZero$Length_bin %in% c('F_20-25cm ', 'F_115-120cm ', 'M_20-25cm', 'M_95-100cm', 'M_100-105cm', 'M_105-110cm', 'M_110-115cm', 'M_115-120cm'), ]
...
# Using JRWToolBox::recode.simple() to re-code correctly via 'Length_bin_num' (hacked together the 'ref_Table' using 'charSort' above, but 'ref_Table' could be created in anyway desired)
   ...
   if(numSexInModel %in% 1)
      Order <- c(7:23, 1:6)
   if(numSexInModel %in% 2)
      Order <- c(5:20, 1:4, 21:34) # Females then males    
...
#      Coastwide      CA      OR       WA
# 2012   634.270 288.579 143.124  666.055
# 2013   170.315 176.920  68.155   96.520
# 2014   384.892 184.812 129.487  417.353
# 2015   400.028 114.203 105.830  514.353
# 2016   255.187 117.479  90.557  198.222
# 2017   168.332 104.723  56.298  165.325
# 2018   229.219 141.654 102.566  151.939
# 2019  1150.042 105.382 103.282 2454.828


dimnames(Proportions$Index_tl) <- list(fit$year_labels, strataLimits$STRATA)
r(Proportions$Index_tl, 3)

#     Coastwide       CA       OR        WA
# 2012 13958.467 3217.555 1145.218  9595.694
# 2013  3318.621 1661.203  404.941  1252.478
# 2014  8869.514 2263.875 1172.326  5433.313
# 2015  5358.055  907.177  547.162  3903.716
# 2016  4619.354  997.233  512.304  3109.816
# 2017  2353.664  693.609  244.605  1415.450
# 2018  5017.429 1291.307  712.344  3013.778
# 2019 47569.052 2714.369 1539.304 43315.379
