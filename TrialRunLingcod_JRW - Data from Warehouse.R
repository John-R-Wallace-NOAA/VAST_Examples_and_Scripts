...
if (!any(installed.packages()[, 1] %in% "SpatialDeltaGLMM")) 
    devtools::install_github("nwfsc-assess/geostatistical_delta-GLMM")

if (!any(installed.packages()[, 1] %in% "VAST")) 
    devtools::install_github("james-thorson/VAST")

if (!any(installed.packages()[, 1] %in% "TMBhelper")) 
     devtools::install_github("kaskr/TMB_contrib_R/TMBhelper")

if (!any(installed.packages()[, 1] %in% "ThorsonUtilities")) 
...
# setwd("C:\\Lingcod\\Data\\NWFSCSurvey\\")
...
# rho config - autocorrelations, defaults to zero, both temporal and spatial
# over dispersion config, vessel effects for both components of the model?
...
(DateFile <- paste(getwd(),'/VAST_output_', Sys.Date(), '_LingCod_nx=', n_x, '/',sep='')) # Change '_LingCod_nx=' for different runs, e.g. '_LingCod_Pass_nx=' for including pass
dir.create(DateFile)
...
Data_Geostat = data.frame(Catch_KG = Data_Set$Wt_kg, Year = Data_Set$Year, Vessel = Data_Set$Vessel, AreaSwept_km2 = Data_Set$Area_Swept_ha/100, Lat =Data_Set$Latitude_dd, 
...
TmbData = VAST::Data_Fn("Version"=Version, "FieldConfig"=FieldConfig, "OverdispersionConfig"=OverdispersionConfig, "RhoConfig"=RhoConfig, "ObsModel"=ObsModel, 
                   "c_i"=rep(0,nrow(Data_Geostat)), "b_i"=Data_Geostat[,'Catch_KG'], "a_i"=Data_Geostat[,'AreaSwept_km2'], "v_i"=as.numeric(factor(Data_Geostat[,'Vessel']))-1, 
...
# Save all
save.image(paste0(DateFile, "Image.Rdata")) # Save an image so everything is not stuck under Save - JRW
...
SpatialDeltaGLMM::PlotResultsOnMap_Fn(plot_set= 4, MappingDetails=MapDetails_List[["MappingDetails"]], Report=Report, Sdreport=Opt$SD, PlotDF=MapDetails_List[["PlotDF"]], 
